...
//CSurfaceRendererInOpenGL cSurfaceRenderer;
CSurfaceRendererBasic cSurfaceRenderer;
float from[3], to[3]; 
...
//	}
//	// ADD-BY-LEETEN 07/07/2010-END
//	cSurfaceRenderer._Update();
//}

void draw_streamsurface() 
{
	glPushAttrib(
		GL_LIGHTING_BIT |
		0
...
{  
	...
	startX = center[0] - len[0] * 0.25;
	startY = center[1];
	startZ = center[2];

	endX = center[0] + len[0] * 0.25;
	endY = center[1];
	endZ = center[2];
	float rake_length = 50;
	int nSeeds = 16; 
	VECTOR3* seeds;
	int circle=0,step=0;
	list<VECTOR3> mylist;
	TRACE_DIR direction=FORWARD_DIR;
	float ribbon_length = 100;

	float dx,dy,dz;
	//int split_num=1.0/1.0;
	dx=(endX-startX)/(nSeeds - 1);
	dy=(endY-startY)/(nSeeds - 1);
	dz=(endZ-startZ)/(nSeeds - 1);
	...
	_KeyboardFunc(unsigned char ubKey, int iX, int iY)
{
	...
	case 'h':
		{
			int iHalo;
			cSurfaceRenderer._GetInteger(CSurfaceRenderer::ENABLE_HALO, &iHalo);
			iHalo = !iHalo;
			cSurfaceRenderer._SetInteger(CSurfaceRenderer::ENABLE_HALO, iHalo);
		}
		glutPostRedisplay();
		break;
	...
	case 'l':
		{
			int iLighting;
			cSurfaceRenderer._GetInteger(CSurfaceRenderer::ENABLE_LIGHTING, &iLighting);
			iLighting = !iLighting;
			cSurfaceRenderer._SetInteger(CSurfaceRenderer::ENABLE_LIGHTING, iLighting);
		}
	...
	init()
{
	...
	cSurfaceRenderer._UpdateLighting();
	...
	main(int argc, char* argv[])
{
	...
	///////////////////////////////////////////////////////////////
	cSurfaceRenderer._SetBoundingBox(
		minLen[0], minLen[1], minLen[2], 
		maxLen[0], maxLen[1], maxLen[2]);
	cSurfaceRenderer._SetDataSource(&mylocallist);
	// ADD-BY-LEETEN /2010-BEGIN
	cSurfaceRenderer._SetColorSource(&liv4Colors);
	cSurfaceRenderer._SetInteger(CSurfaceRenderer::COLOR_SCHEME, CSurfaceRenderer::CColorScheme::COLOR_PER_TRACE);
	// ADD-BY-LEETEN 07/07/2010-END
